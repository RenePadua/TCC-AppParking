...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($placa == 'all') {
        // Query para exibir todos os registros sem filtro, excluindo os veículos sem saída registrada
        $query = "SELECT r.cd_Registro, v.cd_Veiculo, v.cd_Placa, t.nm_Tipo, m.nm_Marca, v.nm_Modelo, v.nm_Cor, r.dt_Entrada, r.dt_Saida, r.vl_Pagamento FROM registro r INNER JOIN veiculo v ON r.cd_Veiculo = v.cd_Veiculo JOIN marca m ON v.cd_Marca = m.cd_Marca JOIN tipo t ON v.cd_Tipo = t.cd_Tipo WHERE r.dt_Saida IS NOT NULL ORDER BY r.dt_Saida DESC";
    } else {
        // Query para filtrar por placa, excluindo os veículos sem saída registrada
        $query = "SELECT r.cd_Registro, v.cd_Veiculo, v.cd_Placa, t.nm_Tipo, m.nm_Marca, v.nm_Modelo, v.nm_Cor, r.dt_Entrada, r.dt_Saida, r.vl_Pagamento FROM registro r INNER JOIN veiculo v ON r.cd_Veiculo = v.cd_Veiculo JOIN marca m ON v.cd_Marca = m.cd_Marca JOIN tipo t ON v.cd_Tipo = t.cd_Tipo WHERE  v.cd_Placa = '$placa' AND r.dt_Saida IS NOT NULL ORDER BY r.dt_Saida DESC";
    ...
    <table class="table align-middle text-center table-bordered table-hover border-dark-subtley table-striped">
      ...
      <tbody>
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo "<td>" . $saidaFormatada . "</td>";
			
			$valorSaida = $row['vl_Pagamento'];
			//Formata o valor calculado para versão monetária
			$valorSaidaFormatado = 'R$ ' . number_format($valorSaida, 2, ',', '.');
			echo "<td>" . $valorSaidaFormatado . "</td>";
            echo "</tr>";
        }
        ?>
      </tbody>
      ...
      <form method="POST" action="historico.php">
        <div class="modal-body">
          <div class="form-group col-md-6">
            ...
            $query1 = "SELECT DISTINCT v.cd_Placa 
						FROM veiculo v 
